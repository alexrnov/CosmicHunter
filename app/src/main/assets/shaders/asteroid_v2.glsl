...
layout(location = 0) attribute vec4 a_position; // сюда загружаются данные вершин
layout(location = 1) attribute vec2 a_textureCoordinates; // сюда загружаются двухкомпонентные текстурные координаты
layout(location = 2) attribute vec3 a_normal; // сюда загружаются нормали
...
smooth varying vec4 v_commonLight; //интерполятор для общего освещения(фоновое + диффузное)
