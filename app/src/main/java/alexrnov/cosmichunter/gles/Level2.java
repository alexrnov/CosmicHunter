...
public class Level2 implements GLSurfaceView.Renderer, SceneRenderer {
  ...
  private int numbersOfRockets = 150; // количество оставшихся ракет
  ...
  // астероиды и летящие ракеты помещаются в отдельную коллекцию,
  // которая сортируется по дальности объектов. Это нужно для того, чтобы
  // ракеты были видны за астероидами.
  private List<Object3D> transparentObject = new ArrayList<>();
  ...
  private Comparator<Object3D> comparatorByZ;

  public Level2(double versionGL, Context context) {
    //Log.i(TAG, "versionGL = " + versionGL + ", Level2");
  ...
  public void onSurfaceCreated(GL10 glUnused, EGLConfig config) {
    ...
    */
    ...
    asteroids[0] = new IceAsteroid(versionGL, context, 1.0f, "objects/asteroid1.obj");
    bindExplosions(asteroids[0], activeExplosions, versionGL, context);
    asteroids[1] = new IceAsteroid(versionGL, context, 1.0f, "objects/asteroid2.obj");
    bindExplosions(asteroids[1], activeExplosions, versionGL, context);
    asteroids[2] = new IceAsteroid(versionGL, context, 1.0f, "objects/asteroid3.obj");
    bindExplosions(asteroids[2], activeExplosions, versionGL, context);
    asteroids[3] = new IceAsteroid(versionGL, context, 1.0f, "objects/asteroid4.obj");
    bindExplosions(asteroids[3], activeExplosions, versionGL, context);

    asteroids[4] = new IceAsteroid(versionGL, context, 1.0f, "objects/asteroid5.obj");
    bindExplosions(asteroids[4], activeExplosions, versionGL, context);
    asteroids[5] = new IceAsteroid(versionGL, context, 1.0f, "objects/asteroid6.obj");
    bindExplosions(asteroids[5], activeExplosions, versionGL, context);
    asteroids[6] = new IceAsteroid(versionGL, context, 1.0f, "objects/asteroid7.obj");
    ...
    backgroundObject3D = new Background(versionGL, context, 160.0f, R.raw.sky_texture_level2);
  ...
  public void onSurfaceChanged(GL10 glUnused, int width, int height) {
    ...
    for (Asteroid asteroid: asteroids) {
      transparentObject.add((Object3D) asteroid);
    }

    comparatorByZ = (object1, object2) -> { // сортировать объекты по глубине
      Float z1 = object1.getView().getZ();
      Float z2 = object2.getView().getZ();
  ...
  public void onDrawFrame(GL10 glUnused) { // вызывается при перерисовке кадра
      ...
      if (asteroid.getView().checkHit(flyRockets)) {
        ...
        if (numberOfHints == needNumberOfHints && !gameActivity.timeOver) {
          AsyncTask.execute(() -> {
            ...
            dao.updateLevel("level3", true); // открыть следующий уровень
          });
    ...
    // отсортировать все полупрозрачные объекты (астероиды и летящие ракеты)
    // по дальности. Если этого не сделать, тогда за астероидами
    // не будут отображатся ракеты.
    Collections.sort(transparentObject, comparatorByZ);
    for (Object3D object3D: transparentObject) object3D.draw();

    /* отрисовка только активных взрывов */
    for (Explosion explosion: activeExplosions) explosion.draw(delta);

    // удалить текущий список летящих ракет из коллекции полупрозрачных объектов
    transparentObject.removeAll(flyRockets);
  ...
  private void defineFlyRocket() {
    ...
    // добавить в коллекцию полупрозрачных объектов текущий список
    // летящих ракет
    transparentObject.addAll(flyRockets);
  }
  ...
  public void setPassXY(Float passX, Float passY) {
    /*
     * На этом уровне не используется лямда. Это было сделано для проверки,
     * того, что быстрее условный оператор или лямда. Замеры времени показали
     * примерно одинаковые результаты. Здесь переменная функционального
     * интрерфейса не используется - и как следствие происходит экономия памяти
     */
    if (loadGame && numbersOfRockets > 0) {
