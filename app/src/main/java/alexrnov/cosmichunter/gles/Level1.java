...
public class Level1 implements GLSurfaceView.Renderer, SceneRenderer {
  ...
  private int numbersOfRockets = 900; // количество оставшихся ракет
  ...
  public Level1(double versionGL, Context context) {
    //Log.i(TAG, "versionGL = " + versionGL + ", Level1");
  ...
  public void onSurfaceCreated(GL10 glUnused, EGLConfig config) {
    ...
    // реализация делает предпочтение на точность
    ...
    */
    ...
    asteroids[0] = new BasaltAsteroid(versionGL, context, 1.0f, "objects/asteroid1.obj");
    bindExplosions(asteroids[0], activeExplosions, versionGL, context);
    asteroids[1] = new BasaltAsteroid(versionGL, context, 1.0f, "objects/asteroid2.obj");
    bindExplosions(asteroids[1], activeExplosions, versionGL, context);
    asteroids[2] = new BasaltAsteroid(versionGL, context, 1.0f, "objects/asteroid3.obj");
    bindExplosions(asteroids[2], activeExplosions, versionGL, context);
    asteroids[3] = new BasaltAsteroid(versionGL, context, 1.0f, "objects/asteroid4.obj");
    bindExplosions(asteroids[3], activeExplosions, versionGL, context);

    asteroids[4] = new BasaltAsteroid(versionGL, context, 1.0f, "objects/asteroid5.obj");
    bindExplosions(asteroids[4], activeExplosions, versionGL, context);
    asteroids[5] = new BasaltAsteroid(versionGL, context, 1.0f, "objects/asteroid6.obj");
    bindExplosions(asteroids[5], activeExplosions, versionGL, context);
    asteroids[6] = new BasaltAsteroid(versionGL, context, 1.0f, "objects/asteroid7.obj");
    ...
    backgroundObject3D = new Background(versionGL, context, 160.0f, R.raw.sky_texture_level1);
  ...
  public void onDrawFrame(GL10 glUnused) { // вызывается при перерисовке кадра
    ...
    for (Asteroid asteroid: asteroids) {
      ...
      if (asteroid.getView().checkHit(flyRockets)) {
        ...
        // если достигнуто достаточное количество попаданий
        // и время еще не вышло - открыть следующий уровень
        ...
        if (numberOfHints == needNumberOfHints && !gameActivity.timeOver) {
          AsyncTask.execute(() -> {
            ...
            dao.updateLevel("level2", true); // открыть следующий уровень
          });
